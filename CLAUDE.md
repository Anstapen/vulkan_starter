...
- Configurations: `Debug`, `Release`, `Dist`. `Ping` builds `warnings "Extra"` and (Release/Dist) `fatalwarnings "All"` — warnings in `Ping/Source` will break non-Debug builds.
- Shaders are compiled manually, not as part of the main build: `Examples/Shaders/compile.bat` invokes `slangc` on `shader.slang` to produce `slang.spv` (targets SPIR-V 1.4, entry points `vertMain`/`fragMain`).

## Architecture

Two-project split: `Ping` (static lib, the publicised RHI — the only project meant to be consumed by other projects) and `Examples` (executable demonstrating `Ping`; currently just `Examples/Source/App.cpp` constructing and running `Mupfel::Application`). `Examples` links `Ping`, `vulkan`, `glfw3`, `spdlog`, `imgui`, and includes `Ping/Source` for `Ping`'s public headers.

`Ping/Source` is layered into two namespaces that must not be conflated:

- **`Ping::`** (`Ping/Source/Ping/`) — the public, backend-agnostic RHI surface: `Device`, `SwapChain`, `Pipeline`, `CommandBuffer`, `Buffer`, and the plain-data types/enums in `Types.h` (`ImageLayout`, `BufferUsage`, `MemoryProperty`, etc.). Nothing outside `Ping/Source/Vulkan` should need to include Vulkan headers directly — code should go through `Ping` types. This layer is intentionally windowing-library-agnostic: `Device`/`SwapChain` take a raw `GLFWwindow*` (forward-declared, not `#include`d) rather than depending on any concrete window wrapper, so `Ping` has no dependency on the `Window` class living in `Examples`.
- **`Backend::`** (`Ping/Source/Vulkan/`) — the Vulkan implementation of the `Ping` interface, built on `vk::raii` (vulkan-hpp RAII wrappers). `VKManager` is a static-method class holding essentially all raw Vulkan logic (instance/device creation, swapchain, pipeline, command buffers/pools, buffers, memory type selection, layout transitions, dynamic rendering setup). `VulkanContext`, `VulkanSwapChain`, `VulkanPipeline`, `VulkanCommandBuffer`, `VulkanCommandPool`, `VulkanQueue`, `VulkanBuffer` are the RAII-owning counterparts behind each `Ping` type. `VulkanTypeConversions.h/.cpp` holds the `Backend::ToVulkan(Ping::X)` mapping functions that bridge the two namespaces — new `Ping` enum values need a corresponding conversion here.

`Ping/Source/Logger/` is a thin `Mupfel::Logger` wrapper around spdlog, shared by both projects — `Backend::VKManager` logs through it internally (validation-layer warnings, device enumeration, etc.), which is why it lives in `Ping` rather than `Examples` despite being generic (non-RHI-specific) code.

`Examples/Source` holds everything built on top of `Ping` rather than part of the RHI itself, under the `Mupfel::` namespace (`Application/`, `Renderer/`) plus the plain `Window` class:

- **`Window`** (`Examples/Source/Window/`) — a small RAII GLFW window wrapper. Not part of `Ping`; call sites into `Ping` extract the raw handle via `Window::GetGLFWHandle()`.
- **`Mupfel::`** (`Examples/Source/Application/`, `Examples/Source/Renderer/`) — the application/engine layer built on top of `Ping`. `Application` owns the `Window`, `Ping::Device`, `Renderer`, and `World`, and drives `Init()`/`MainLoop()`. `Renderer::RenderNextFrame` is the per-frame entry point, tracking its own swapchain/pipeline/command buffers/vertex buffers and a fixed `frames_in_flight = 2`.

`Examples/Source/MupfelDeps/` is a vendored ECS + event system used by the `Mupfel` layer (not part of the RHI):
...
- Classes, methods, and free functions use a `/** */` block. The first sentence becomes the brief
   ...
   *
   * @param in_window The GLFW window handle to present to.
