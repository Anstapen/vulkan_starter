...
public:
	...
	auto GetEvents() const noexcept
		-> std::ranges::subrange<typename EventBuffer<T>::const_iterator, typename EventBuffer<T>::const_iterator>;
	 ...
	 */
	template <typename T> void RegisterListener(std::function<void(const T&)> callback);
...
private:
	 ...
	 * @return index.
	 */
	template <typename T> static size_t EventIndex() noexcept
	{
...
template <typename T>
	requires std::derived_from<T, Event>
inline auto EventSystem::GetEvents() const noexcept
	-> std::ranges::subrange<typename EventBuffer<T>::const_iterator, typename EventBuffer<T>::const_iterator>
{
...
template <typename T> inline void EventSystem::RegisterListener(std::function<void(const T&)> callback)
{
	size_t index = EventIndex<T>();
	listeners[index].push_back([cb = std::move(callback)](const Event& evt) { cb(static_cast<const T&>(evt)); });
}
